...
#
# NOTE: this version replaces every `jq` call with an inline `python3 -c`
# equivalent — jq is no longer a prerequisite for this package.
# =============================================================================
set -euo pipefail
...
poll_fabric_lro() {
  ...
  while [[ "$status" == "Running" || "$status" == "NotStarted" ]]; do
    ...
    status=$(echo "${resp}" | python3 -c 'import json,sys; d=json.load(sys.stdin); print(d.get("status","Unknown"))')
...
# Polls a Fabric NOTEBOOK JOB run until it finishes. Job status values
# ("NotStarted", "InProgress", "Completed", "Failed", "Cancelled") are a
# different vocabulary than the resource-creation LRO's ("Running",
# "Succeeded", "Failed") — that's why this is a separate function from
# poll_fabric_lro above, not a shared one.
# Usage: poll_notebook_job "<job-instance-status-url>"
# Prints the final job JSON to stdout; exits non-zero if the job failed.
poll_notebook_job() {
  local status_url="$1"
  local token
  token=$(fabric_token)
  ...
  while [[ "$status" == "NotStarted" || "$status" == "InProgress" || "$status" == "Running" ]]; do
    attempt=$((attempt + 1))
    if [[ $attempt -gt 120 ]]; then
      echo "ERROR: notebook job did not finish after 20 minutes of polling: ${status_url}" >&2
      return 1
    fi
    sleep 10
    resp=$(curl -s -H "Authorization: Bearer ${token}" "${status_url}")
    # status=$(echo "${resp}" | jq -r '.status // "Unknown"')
    status=$(echo "${resp}" | python3 -c 'import json,sys; d=json.load(sys.stdin); print(d.get("status","Unknown"))')
    echo "  ...notebook job status: ${status} (poll ${attempt}/120)" >&2
  done

  if [[ "$status" != "Completed" ]]; then
    echo "ERROR: notebook job finished with status: ${status}" >&2
    echo "${resp}" >&2
    return 1
  fi
